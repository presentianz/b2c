<?php
    ...
    class CategoryRepository extends NestedTreeRepository
    {
        public function getCategoryProducts($id, $page, $sort, $item_no)
        {
            ...
            //find data
            $query_products = $gEM->createQuery('SELECT c.name AS category_name,
                ...
                WHERE c.id IN (:ids) 
                ORDER BY ' . $orderby)->setFirstResult(($page - 1) * $item_no)->setMaxResults($item_no)->setParameter('ids', $ids);
            $products = $query_products->getResult();

            //find total
            $query_total = $gEM->createQuery('SELECT COUNT(p.id) AS total
                FROM AppBundle:Category c JOIN c.products p 
        ...
        public function getRandomProductsUnderCategory($id, $no)
        {
            ...
            $query = $em->createQuery('SELECT
                                    ...
                                    p.imageLink 
                FROM AppBundle:Category c JOIN c.products p 
                WHERE c.id IN (:ids)')->setParameter('ids', $ids)->setMaxResults($no);
        ...
        {
            $em = $this->getEntityManager();
            $query = $em->createQueryBuilder()->select('node')->from('AppBundle:Category', 'node')->orderBy('node.root, node.lft', 'ASC')->where('node.lvl = 0 OR node.lvl = 1')->getQuery();
        ...
        {
            $query = $this->getEntityManager()->createQuery('SELECT c FROM AppBundle:Category c WHERE c.parent = :parentId')->setParameter('parentId', $parentId);
        ...
        {
            $query = $this->getEntityManager()->createQuery('SELECT c FROM AppBundle:Category c WHERE c.parent = :parentId AND c.id != :id')->setParameter('parentId', $parentId)->setParameter('id', $id);
            $category = $query->getResult();
            return $category;
        }
    }
